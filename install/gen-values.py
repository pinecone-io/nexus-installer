...
every field) and emits, into the output dir (default install/generated/):
  ...
  values.install.yaml     top-level overrides the OCI chart's values.yaml cannot carry
                          (staticIndex, global.image/sizing, ingress, nexus.config index/embed).
...
def build_install_values(inp, dim):
    ...
    values = {
        ...
        "global": {"image": image, "sizing": sizing(inp)},
        "nexus": {
            "config": {
...
def main():
    ...
    dump(
        ...
        "# cannot carry (staticIndex, global.image/sizing, ingress, nexus.config index/embed).\n",
    )
    provider = storage_provider(inp)
    if provider == "s3":
        dump(
