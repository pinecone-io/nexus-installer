...
every field) and emits, into the output dir (default install/generated/):
  ...
  values.install.yaml     top-level overrides the OCI chart's values.yaml cannot carry
                          (staticIndex, global.image/sizing, the DB-tier profile the size
                          class selects, ingress, nexus.config index/embed).
...
def dbslim_profile_overlay(size):
    ...
    if not profile:
        return None
    return {
        svc: {"pinecone": {"config_overrides": {DBSLIM_CELL_KEY: {"profile": profile}}}}
...
def build_install_values(inp, dim):
    ...
    values = {
        ...
        "global": {"image": image, "sizing": size},
        "nexus": {
            "config": {
    ...
    dbslim = dbslim_profile_overlay(size)
    if dbslim:
        values["db-slim"] = dbslim
...
def main():
    ...
    dump(
        ...
        "# cannot carry (staticIndex, global.image/sizing, the DB-tier profile the size class\n"
        "# selects, ingress, nexus.config index/embed).\n",
    )
    provider = storage_provider(inp)
    if provider == "s3":
        dump(
