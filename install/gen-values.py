...
every field) and emits, into the output dir (default install/generated/):
  ...
  values.install.yaml     top-level overrides the OCI chart's values.yaml cannot carry
                          (staticIndex + its global.staticIndex mirror, global.image/sizing,
...
def build_install_values(inp, dim):
    ...
    values = {
        ...
        # global.staticIndex; the chart fails the render when the two trees differ.
        "global": {"image": image, "sizing": sizing(inp), "staticIndex": dict(static_index)},
        "nexus": {
            "config": {
...
def main():
    ...
    dump(
        ...
        "# carry (staticIndex + global.staticIndex, global.image/sizing, ingress,\n"
