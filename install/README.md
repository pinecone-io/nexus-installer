...
instead of partway through the install. `--only-live-gateway` runs that check alone — an
inputs file holding just the `inference` and `embedding` blocks and a stock `python3` (that
mode needs no PyYAML) are enough, so it can be handed to whoever holds the credentials.
